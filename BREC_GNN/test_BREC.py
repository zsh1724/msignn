...
part_dict = {
    ...
    "Distance_Regular": (20, 40),
}
#when running other datasets, you should modify the funciton BRECDataset in BRECDataset_v3, the more details should be found in https://github.com/lukeyf/BREC-L to modify the dataset.
# you also need to change the range for the dataset from 0
...
def fmb(i,d_m,vertices_number):
    al=[vertices_number,vertices_number]
    with open ('test.csv','a',newline='') as fo:
        wo=writer(fo)
        wo.writerow(al)
        fo.close()
    co = torch.zeros((vertices_number, 200))
...
def get_metric_basis(edge_index1,num_nodes1):
...
def get_dataset(name, device):
    time_start = time.process_time()
    def makeMSfeatures(data):
        af=get_metric_basis(data.edge_index,data.num_nodes)
        data.x=af
        return data
    # Do something
    ...
    pre_transform = T.Compose([makeMSfeatures, addports])

    dataset = BRECDataset(name=name, pre_transform=pre_transform) # you should change here when you select other dataset
    time_end = time.process_time()
    time_cost = round(time_end - time_start, 2)
...
def evaluation(dataset, path, device, args):
    ...
    for part_name, part_range in part_dict.items():
        ...
        for id in tqdm(range(part_range[0], part_range[1])):
            logger.info(f"ID: {id}")
            model = get_model(args, num_nodes_list[id], 200, device)
    ...
    logger.info(
        "Correct\tnum_layers\thidden_units\tnum_runs\tOUTPUT_DIM\tBATCH_SIZE\tLEARNING_RATE\tWEIGHT_DECAY\tSEED"
    )
    logger.info(
        f"{cnt}\t{args.num_layers}\t{args.hidden_units}\t{args.num_runs}\t{OUTPUT_DIM}\t{BATCH_SIZE}\t{LEARNING_RATE}\t{WEIGHT_DECAY}\t{SEED}"
    )
